...
一个用浏览器打开就能做语法填空的练习工具。内容按**上海命题风格**整理（10 空 = 约 4 空有提示词 + 6 空无提示词，有提示词不考词性转换），初高中都能用，**全国其他地区也可以照常使用**——时态、非谓语、三大从句、介词、冠词、代词这些考点是通用的。默认从高一基础开始。

> 仓库名 / 产物文件名仍是 `english-grammar-lab`、`English-Grammar-Lab.html`，方便已发布的链接继续可用；页面里显示的产品名是「上海初高语法填空」。
...
考点按 11 个大专题组织：谓语动词、非谓语动词、形容词/副词比较等级、定语从句、名词性从句、状语从句、介词、冠词、代词、并列与逻辑、其他拓展。这是上海卷的口径——10 空 = 约 4 空有提示词 + 6 空无提示词；有提示词只考动词变形与形副比较级/最高级，**不考词性转换**（没有 care→careful→carefully 这类题）。

- **内置题库**：谓语动词·时态 01–12 十二个专题，每个专题 12 道核心题加 6 道扩展题，另有 20 道时态综合挑战（12 个专题做完后解锁）。其他大专题没有内置题目，配合 AI 出题用。
- **解题方法课**：把上海卷做题顺序拆成 5 步——先通读数谓语，给词先分"动词 / 形副"两类，无词按"连词 > 介词 > 冠词 > 代词 > 情态"排查，最后通读复查；另有无提示词 5 类候选池、快速排查流程和上海高频坑。
